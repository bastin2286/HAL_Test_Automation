pipeline 
{
        ...
        stage('Deployment to DUT from Host Server') 
	{
		 ...
		 //sh '''ssh \'sachin@196.168.43.144\' docker pull localhost:5000/haldeploy:v2'''   
                 sh '''ssh \'sachin@192.168.43.144\' docker run --name halDeploy -p 36551:36551 -v volHAL:/src haldeploy'''
            }
        }
	...
	stage('Execute test in DUT from Host') 
	{
            ...
            steps  
            {
                 ...
                 sh '''ssh \'sachin@192.168.43.144\' docker run --name halBuild -p 36551:36551 -v volHAL:/src halbuild'''
            }
        }
