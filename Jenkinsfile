pipeline 
{
        ...
        stage('Deployment to DUT from Host Server') 
	{
		 ...
		 sh '''ssh \'318356@10.10.196.130\' docker pull localhost:5000/haldeploy:v2'''   
                 sh '''ssh \'318356@10.10.196.130\' docker run --name halDeploy -p 22:22 -v volHAL:/src localhost:5000/haldeploy:v2'''
            }
        }
	...
	stage('Execute test in DUT from Host') 
	{
            ...
            steps  
            {
                 ...
                 sh '''ssh \'318356@10.10.196.130\' docker run --name halBuild -p 22:22 -v volHAL:/src localhost:5000/haltestbuild:v1'''
            }
        }
